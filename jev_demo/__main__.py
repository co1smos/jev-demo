    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if path == "/":
                ...
                comparison_rows = ""
                parent_runs = [
                    run for run in application.store.list()
                    if run["request"].get("method") == "jev"
                ]
                history_items = "".join(
                    f'''<li>{f'<a href="/?run={escape(run["id"])}">{escape(run["request"]["trading_date"])}</a>' if run["status"] == "completed" else escape(run["request"]["trading_date"])} — {escape(run["status"])}. Created {escape(run["created_at"])}. Updated {escape(run["updated_at"])}.{f' Progress: {run["progress"]["current"]} of {run["progress"]["total"]}.' if run["status"] == "running" else ''}{f' Failure: {escape(run["error"] or "unknown error")}.' if run["status"] == "failed" else ''}</li>'''
                    for run in parent_runs
                ) or "<li>There are no parent JEV runs in this database. Verification evidence used a separate temporary database and was not imported.</li>"
                if selected_id:
                    try:
